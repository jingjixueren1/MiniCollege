...
Page({
	...
	bindShowDetailTap: async function (e) {
		...
		let info = await ccminiCloudHelper.callCloudData('admin/info_detail', params);
		if (!info) {
	...
	bindSortTap: async function (e) {
		...
		let that = this;
		try {
			await ccminiCloudHelper.callCloudSumbit('admin/info_sort', params).then(res => {
				ccminiPageHelper.modifyListNode(id, that.data.dataList.list, 'INFO_ORDER', sort);
				that.setData({
	...
	bindDelTap: async function (e) {
		...
		let callback = async function () {
			try {
				...
				await ccminiCloudHelper.callCloudSumbit('admin/info_del', params, opts).then(res => {
	...
	bindStatusTap: async function (e) {
		...
		let that = this;
		try {
			await ccminiCloudHelper.callCloudSumbit('admin/info_status', params).then(res => {
				ccminiPageHelper.modifyListNode(id, that.data.dataList.list, 'INFO_STATUS', status,'_id');
				that.setData({
					dataList: that.data.dataList
				});
